...
layui.use(['form', 'layedit'], function () {
    ...
    $.ajax({
	    ...
	    data:{
	   	...
	   	success:function(res){
	        console.log(res);
			var html='<object data="http://localhost:8080/LIMS_SSM/img/experiment'+res.el.ehelp+'" type="application/pdf" width="100%" height="500px">'+
			    		'<embed src="http://localhost:8080/LIMS_SSM/img/experiment'+res.el.ehelp+'" type="application/pdf">'+
	        				'<p>This browser does not support PDFs. Please download the PDF to view it: <a href="http://localhost:8080/LIMS_SSM/img/experiment'+res.el.ehelp+'">Download PDF</a>.</p>'+
	    				'</embed>'+
					'</object>';
			$('.pdff').append(html);
	    ...
	    data:{
	   	...
	   	success:function(res){
	        ...
	        for(var i=0;i<res.mes.length;i++){
	            var html = '<li><div class="comment-parent"><img src="http://localhost:8080/LIMS_SSM/img/student'+res.mes[i].uphoto+'" alt="丢啦"/><div class="info"><span class="username">'+res.mes[i].uname+'</span><span class="time">'+layui.util.toDateString(res.mes[i].mtime, "yyyy-MM-dd HH:mm:ss")+'</span></div><div class="content">' + res.mes[i].mbody + '</div></div></li>';
    ...
    form.on('submit(formRemark)', function (data) {
        ...
        setTimeout(function () {
            ...
            var html = '<li><div class="comment-parent"><img src="http://localhost:8080/LIMS_SSM/img/student${student.sphoto}" alt="${student.sname}"/><div class="info"><span class="username">${student.sname}</span><span class="time">'+layui.util.toDateString(new Date(), "yyyy-MM-dd HH:mm:ss")+'</span></div><div class="content">' + content + '</div></div></li>';
            $('.blog-comment').append(html);
            $('#remarkEditor').val('');
